function varargout = TOUT_section_NoTopo(varargin)
%TOUT_section plot section (vertical slice, 1 segment) of thermal model (and input data)
% this version was stripped from calls to 'minvtran'
% to avoid depending on the Mapping Toolbox and keep OCTAVE compatibilty
%
...
%% load data: grid, layers, output volumes
load('../thermal/Tgrid.mat','Tgrid');
load('../thermal/TIN_layers.mat','TIN_layers');
load('../thermal/TOUT_VOL.mat','TOUT_VOL');
...
SectZ = TIN_layers.DefGrid.z(1):SectI_verStep:TIN_layers.DefGrid.z(end);
...
% build meshgrids from 'TIN_layers.DefGrid' struct
[PX,PY] = meshgrid(TIN_layers.DefGrid.y,TIN_layers.DefGrid.x);
% 'TIN_layers.DefGrid' includes edge padding, use Tgrid for volumes
% (edge padding already stripped)
[VX,VY,VZ] = meshgrid(Tgrid.y,Tgrid.x,Tgrid.z);
% get size of edge padding, one direction is enough
% needed afterwards, to extract Q0 and Qm profile (edges already stripped)
% using SectX, SectY coordinates
PadSize = (length(TIN_layers.DefGrid.x) - length(Tgrid.x))/2;
...
InP.AIR = interp2(...
    PX,PY,...
    TIN_layers.AIR.RoundedDepthMap+TIN_layers.DefGrid.zshift ,...
    SectY,SectX,Interp2Method)*1e-3;
InP.SED = interp2(...
    PX,PY,...
    TIN_layers.SEDS.RoundedDepthMap+TIN_layers.DefGrid.zshift ,...
    SectY,SectX,Interp2Method)*1e-3;
InP.UCR = interp2(...
    PX,PY,...
    TIN_layers.UCRUST.RoundedDepthMap+TIN_layers.DefGrid.zshift ,...
    SectY,SectX,Interp2Method)*1e-3;
InP.LCR = interp2(...
    PX,PY,...
    TIN_layers.LCRUST.RoundedDepthMap+TIN_layers.DefGrid.zshift ,...
    SectY,SectX,Interp2Method)*1e-3;
InP.LAB = interp2(...
    PX,PY,...
    TIN_layers.LID.RoundedDepthMap+TIN_layers.DefGrid.zshift ,...
    SectY,SectX,Interp2Method)*1e-3;

%% extract heat flow profiles from maps
% Q0 and Qm are already stripped of edge padding
Interp2QMethod = 'linear';
InQ.Q0 = interp2(...
    PX(PadSize+1:end-PadSize,PadSize+1:end-PadSize),...
    PY(PadSize+1:end-PadSize,PadSize+1:end-PadSize),...
    TOUT_VOL.Q0,...
    SectY(PadSize+1:end-PadSize),SectX(PadSize+1:end-PadSize),...
    Interp2QMethod);
InQ.Qm = interp2(...
    PX(PadSize+1:end-PadSize,PadSize+1:end-PadSize),...
    PY(PadSize+1:end-PadSize,PadSize+1:end-PadSize),...
...
TOUT_VOL.Ai = TOUT_VOL.A;
TOUT_VOL.ki = TOUT_VOL.k;
TOUT_VOL.Ai(TOUT_VOL.A==99) = 0.02e-6; % under LAB
TOUT_VOL.Ai(TOUT_VOL.A==88) = 0; % over TOPO
TOUT_VOL.ki(TOUT_VOL.k==0) = 2.45; % approx value of k(T) at LAB
...
% plots
% topography: blank in 'NoTopo' version
Ax.S_topo.Visible = 'off';

% heat flow, Q0
Pl.S_Q.Q0 = plot(Sect_d_km(PadSize+1:end-PadSize),InQ.Q0,...
...
% heat flow, Qm
Pl.S_Q.Qm = plot(Sect_d_km(PadSize+1:end-PadSize),InQ.Qm,...
...
if DoColumns~=0
    ...
    TOUT_VOL.k(TOUT_VOL.k==0) = NaN;
    TOUT_VOL.A(TOUT_VOL.A>=88) = NaN;
    ...
    for ColN=1:length(Col_d)
        ...
        ExtractedCols.A(:,ColN) = interp3(VX,VY,VZ,...
            permute(TOUT_VOL.A,VolPermuteOrder),...
